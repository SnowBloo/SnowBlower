...
camera.framerate = 30
rawCapture = PiRGBArray(camera, size=(1366, 768))
...
def line_length(results):
	
	(ptA, ptB, ptC, ptD) = results[0].corners
	ptA = (ptA[0], ptA[1])
	ptB = (ptB[0], ptB[1])

	return math.dist(ptA, ptB)
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	if key == ord("c"):
		...
		cv2.imwrite("distance_" + distance + " ft_" + "length_" + str(line_length(results)) + " px.jpg" , image)
